...
func _process(_delta: float) -> void:
	var player_chunk_position := player.global_position as Vector2i / grid_size / chunk_size
...
# HACK: world generation should be general, honestly this whole script is a hack
...
func _generate_chunk(at_chunk: Vector2i) -> void:
	for x in range(chunk_size.x):
		for y in range(chunk_size.y):
			...
			# HACK: generation should not be completely random and be controlled such that faults are
			# spread evenly
...
func _clear_chunk(at_chunk: Vector2i) -> void:
	...
	var to_remove = (entity_container
					.get_children()
					.filter(func(obj): return obj is Fault)
					.filter(func(fault): return fault_check(fault, min_point, max_point)))
