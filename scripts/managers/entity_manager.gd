...
## Enum of entities that are spawned from scenes.
enum Entity {
	PLAYER,
	ENEMY,
	TURRET,
...
const _ENTITY_PACKED_SCENE: Dictionary[Entity, PackedScene] = {
	Entity.PLAYER: preload("res://scenes/player.tscn"),
	Entity.ENEMY: preload("res://scenes/enemy.tscn"),
	Entity.TURRET: preload("res://scenes/turret.tscn"),
	Entity.BULLET: preload("res://scenes/bullet.tscn"),
	Entity.EXPLOSION: preload("res://scenes/explosion.tscn"),
	Entity.LOOT: preload("res://scenes/loot.tscn"),
	Entity.FAULT: preload("res://scenes/fault.tscn"),
}

const _CLASS_ENTITY: Dictionary[String, Entity] = {
...
func add_entity(entity: Node2D, source: Node) -> void:
	if source.is_multiplayer_authority():
		if is_multiplayer_authority():
			...
			_add_remote_entity.rpc_id(1, _CLASS_ENTITY[entity.get_script().get_global_name()],
					entity.save_scene())
...
@rpc("any_peer", "call_local", "reliable")
func _add_remote_entity(entity_scene: Entity, data: PackedByteArray) -> void:
	if is_multiplayer_authority():
		var entity = _ENTITY_PACKED_SCENE[entity_scene].instantiate()
